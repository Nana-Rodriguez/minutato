...
export default function GuardaHistorialPage() {
  const { currentUser, users, environments, requests } = useAppStore()
  ...
  // Get finalized or rejected requests
  const processedRequests = useMemo(() => {
    return requests.filter(r => ['FINALIZADA', 'RECHAZADA', 'APROBADA'].includes(r.estado))
      .map(r => {
        ...
        <CardContent className="p-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Buscar por funcionario o ambiente..."
                value={search}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-[#E5E7EB]">
                      ...
                      <th className="text-left py-3 px-4 text-sm font-medium text-[#6B7280] hidden md:table-cell">Ambiente</th>
                      <th className="text-left py-3 px-4 text-sm font-medium text-[#6B7280]">Resultado</th>
                    ...
                    {paginatedRequests.map((request) => (
                      <tr key={request.id} className="border-b border-[#E5E7EB] hover:bg-[#F0FDF4] transition-colors">
                        ...
                        <td className="py-3 px-4 text-sm text-[#111827] hidden md:table-cell">
                          Piso {request.environment?.piso} - {request.environment?.numeroAmbiente}
                        </td>
                        <td className="py-3 px-4">
